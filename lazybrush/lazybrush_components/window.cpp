...
bool
lzwindow::eventFilter(QObject *o, QEvent *e)
{
...
int
lzwindow::pressure_to_radius_(qreal pressure) const
{
...
QPoint
lzwindow::transform_pen_position_(const QPoint &pen_position) const
{
    QPointF const widget_center(widget_container_image_->width() / 2, widget_container_image_->height() / 2);
...
QPoint
lzwindow::point_type_to_QPoint(point_type const & point)
{
    return QPoint(point.x(), point.y());
}

QRect
lzwindow::rect_type_to_QRect(rect_type const & rect)
{
    return QRect(rect.x(), rect.y(), rect.width(), rect.height());
}

point_type
lzwindow::QPoint_to_point_type(QPoint const & point)
{
    return point_type(point.x(), point.y());
}

rect_type
lzwindow::QRect_to_rect_type(QRect const & rect)
{
